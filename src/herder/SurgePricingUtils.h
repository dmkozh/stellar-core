...
// Interface for a stack-based container holding transactions.
// This is needs to be implemented for any user of `SurgePricingPriorityQueue`,
// as it operates on stacks and not bare transactions.
class TxStack
{
  public:
...
//   the lines must fit into it. Transactions from this lane can take place of
...
class SurgePricingLaneConfig
{
  public:
    ...
    // Returns a function that assigns a lane to a transaction.
    virtual TxLaneClassifier getLaneClassifier() const = 0;
...
class DexLimitingLaneConfig : public SurgePricingLaneConfig
{
  public:
    ...
    TxLaneClassifier getLaneClassifier() const override;
...
class SurgePricingPriorityQueue
{
  public:
    ...
    // Result of visiting the transaction stack in the queue.
    ...
    // highest fee rate transactions is consider to be at the top, otherwise the
    // lowest fee rate transaction is at the top.
  ...
  private:
    ...
    std::shared_ptr<SurgePricingLaneConfig> const mLaneConfig;
    std::vector<uint32_t> const& mLaneOpsLimits;
    SurgePricingLaneConfig::TxLaneClassifier const mLaneClassifier;
