    ...
    SECTION("soroban txs")
    {
        ...
        SECTION("soroban surge pricing, classic unaffected")
        {
            ...
            auto const& classicTxs =
                txSet->getTxsForPhase(TxSetPhase::CLASSIC).getNonParallelTxs();
            REQUIRE(classicTxs.size() == 1);
            REQUIRE(classicTxs[0]->getFullHash() == tx->getFullHash());
            auto const& sorobanTxs =
                txSet->getTxsForPhase(TxSetPhase::SOROBAN).getNonParallelTxs();
    ...
    SECTION("validateValue closeTimes")
    {
        ...
        auto testTxBounds = [&](TimePoint const minTime,
            ...
            TxSetUtils::trimInvalid(
                applicableTxSet->getTxsForPhase(TxSetPhase::CLASSIC)
                    .getNonParallelTxs(),
...
TEST_CASE("do not flood invalid transactions", "[herder]")
{
    ...
    auto [_, applicableTxSet] =
        makeTxSetFromTransactions(PerPhaseTransactionList{txs}, *app, 0, 0);
    REQUIRE(applicableTxSet->sizeTxTotal() == 1);
    REQUIRE((*applicableTxSet->getTxsForPhase(TxSetPhase::CLASSIC).begin())
