...
class TxQueueLimiter
{
    ...
    UnorderedMap<TransactionFrameBasePtr, SurgePricingQueueTxBasePtr>
        mStackForTx;
  ...
  public:
...
#ifdef BUILD_TESTS
    size_t size() const;
    std::pair<bool, int64> canAddTx(
        TransactionFrameBasePtr const& tx, TransactionFrameBasePtr const& oldTx,
    ...
    void evictTransactions(
        std::vector<std::pair<SurgePricingQueueTxBasePtr, bool>> const&
            txsToEvict,
