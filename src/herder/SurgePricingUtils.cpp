...
SurgePricingPriorityQueue::SurgePricingPriorityQueue(
    ...
    , mLaneClassifier(mLaneConfig->getLaneClassifier())
...
SurgePricingPriorityQueue::add(TxStackPtr txStack)
{
    ...
    auto lane = mLaneClassifier(*txStack->getTopTx());
    ...
    std::vector<bool>& hadTxNotFittingLane)
{
    ...
    hadTxNotFittingLane.resize(mLaneOpsLimits.size());
    while (true)
    {
        auto currIt = getTop();
        ...
        while (!currIt.isEnd())
        {
            ...
            auto lane = mLaneClassifier(currTx);
    ...
    std::vector<std::pair<TxStackPtr, bool>>& txStacksToEvict) const
{
    ...
    while (neededTotalOps > 0 || neededLaneOps > 0)
    {
        ...
        while (!iter.isEnd())
        {
            ...
            auto evictLane = mLaneClassifier(evictTx);
...
SurgePricingLaneConfig::TxLaneClassifier
DexLimitingLaneConfig::getLaneClassifier() const
{
    return [this](TransactionFrameBase const& tx) { return getTxLane(tx); };
}
...
size_t
DexLimitingLaneConfig::getTxLane(TransactionFrameBase const& tx) const
{
