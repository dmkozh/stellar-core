...
TEST_CASE_VERSIONS("pathpayment strict send", "[tx][pathpayment]")
{
    ...
    // Closes a ledger applying `txs` in the given order. Every transaction is
    // required to be valid against the last closed ledger, so that only the
    // effect of the preceding ones can make any of them fail.
    auto closeLedgerInOrder =
        [&](std::vector<TransactionTestFramePtr> const& txs) {
            std::vector<TransactionFrameBasePtr> baseTxs;
            for (auto const& tx : txs)
            {
                REQUIRE(tx->checkValid(app->getAppConnector(),
                                       ...
                                       0)
                            ->isSuccess());
                baseTxs.emplace_back(tx);
            }
            return closeLedger(*app, baseTxs, /* strictOrder */ true);
        };
    ...
    SECTION("destination is issuer and does not exist for simple paths")
    {
        ...
        for_versions_from(12, *app, [&] {
            ...
            auto resultSet = closeLedgerInOrder({mergeTx, pathPaymentTx});
