           ...
           bool checkSeqNum)
{
    ...
    // std::optional<AccountEntry> srcAccountBefore;
    // bool feeSourceExists = false;
    // {
    //     LedgerTxn ltx(app.getLedgerTxnRoot());
    //     ledgerVersion = ltx.loadHeader().current().ledgerVersion;

    //     // Validate a copy of the transaction, so that `tx` is left untouched
    ...
    //                                                       tx->getEnvelope()));
    //     validationOk = checkedTx->checkValidForTesting(app.getAppConnector(),
    //                                                    ltx, 0, 0, 0);
    //     validationResult = checkedTx->getResult();
    //     REQUIRE((!validationOk || validationResult.result.code() ==
    //     txSUCCESS));

    //     {
    //         auto srcAccount = stellar::loadAccount(ltx, tx->getSourceID());
    //         if (srcAccount)
    //         {
    //             srcAccountBefore = srcAccount.current().data.account();
    //         }
    //     }
    //     // The fee source account is the only account that is strictly
    //     // necessary for the transaction to be a part of a ledger.
    //     feeSourceExists = tx->getFeeSourceID() == tx->getSourceID()
    //                           ? srcAccountBefore.has_value()
    //                           : static_cast<bool>(stellar::loadAccount(
    //                                 ltx, tx->getFeeSourceID()));
    // }

    // if (!feeSourceExists)
    // {
    //     // The transaction can't be a part of a ledger, as the fee can't be
    //     // charged without the fee source account, so the validation result
    //     is
    //     // its only observable outcome. An empty ledger is still closed, so
    //     that
    //     // the ledger sequence advances in the same way as it does for the
    //     // applied transactions.
    //     closeLedger(app);
    //     tx->overrideResultXDR(validationResult);
    //     return false;
    // }
    ...
    // A transaction that is invalid as per `checkValid` has to fail during the
    // application in exactly the same way.
    // if (!validationOk)
    // {
    //     if (tx->getResultCode() != txFAILED)
    //     {
    //         REQUIRE(validationResult == tx->getResult());
    //     }
    //     else
    //     {
    //         auto const& txResults = tx->getResult().result.results();
    //         auto const& validationResults =
    //         validationResult.result.results(); for (auto i = 0u; i <
    //         txResults.size(); i++)
    //         {
    //             REQUIRE(validationResults[i] == txResults[i]);
    //             if (validationResults[i].code() == opBAD_AUTH)
    //             {
    ...
    // if (srcAccountBefore)
    // {
    // The sequence number is not consumed if the transaction fails before
    // or during the sequence number processing. Note, that the transaction
    // may fail during the application even if it has passed the validation
    // against the last closed ledger.

    // bool earlyFailure = (code == txMISSING_OPERATION || code == txTOO_EARLY
    // ||
    ...
    //     stellar::loadAccount(ltx, srcAccountBefore->accountID);
    // if (srcAccountAfter)
    // {
...
applyTx(TransactionTestFramePtr const& tx, Application& app, bool checkSeqNum)
{
    ...
    std::cerr << "tx result: " << xdr::xdr_to_string(tx->getResult())
              << std::endl;
